...
public class Mov_AR_AB extends Thread{
    ...
    public Mov_AR_AB(String direccion, int cantidadMover, String  [][] matrizStr_Logica, JButton [][] matrizBtn_Casillas, 
        ...
        try {
            while(m<cantidadMover){
                if(direccion.equals(Constante.ABA)){
                    if(!matrizStr_Logica[fila+1][col].equals(jugadorDescanso)){
                        //<editor-fold defaultstate="collapsed" desc="A">
                        if(matrizStr_Logica[fila+1][col].equals(Constante.M)){ piezaDano(1); }
                        if(matrizStr_Logica[fila+1][col].equals(Constante.V) && (jugadorActual.getVidaJugador1()<5 &&
                                jugadorActual.getVidaJugador1()>0)){ piezaVida(); }
                        //Mueve Jugador - LOGICO
                        matrizStr_Logica[fila+1][col] = jugadorTurno;
                        matrizStr_Logica[fila][col] = "";
                        //Mueve Jugador - GRAFICO
                        matrizBtn_Casillas[fila+1][col].setIcon(matrizBtn_Casillas[fila][col].getIcon());
...
//</editor-fold>
                        ...
                        if(random12 == 0 ){ 
                            System.out.println("***Inicia subHilo moviendo IZ_DE (izquierda)con mov faltantes: "+(cantidadMover-m));
                            Mov_IZ_DE mov_Izquierda = new Mov_IZ_DE(Constante.IZQ, (cantidadMover-m), matrizStr_Logica, matrizBtn_Casillas, listaVidas, jugadorActual, jugadorTurno, tam);
                            mov_Izquierda.start();
                            this.listaVidas = mov_Izquierda.getListaVidas();
                            this.jugadorActual = mov_Izquierda.getJugadorActual();
                            this.matrizStr_Logica = mov_Izquierda.getMatrizStr_Logica();
                            this.matrizBtn_Casillas = mov_Izquierda.getMatrizBtn_Casillas();
                            m=cantidadMover;
                        } else { 
                            System.out.println("***Inicia subHilo moviendo IZ_DE (derecha) con mov faltantes: "+(cantidadMover-m));
                            Mov_IZ_DE mov_Derecha = new Mov_IZ_DE(Constante.DER, (cantidadMover-m), matrizStr_Logica, matrizBtn_Casillas, listaVidas, jugadorActual, jugadorTurno, tam);
                            mov_Derecha.start();
                            this.listaVidas = mov_Derecha.getListaVidas();
                            this.jugadorActual = mov_Derecha.getJugadorActual();
                            this.matrizStr_Logica = mov_Derecha.getMatrizStr_Logica();
                            this.matrizBtn_Casillas = mov_Derecha.getMatrizBtn_Casillas();
                            m=cantidadMover;
                        }
                    }
                }
